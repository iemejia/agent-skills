<!-- Source: Trevin Chow, "10 Principles for Agent-Native CLIs"
     https://x.com/trevin/status/2051316002730991795 (May 4, 2026)
     Captured verbatim for offline reference. -->

# 10 Principles for Agent-Native CLIs

Last month I wrote about 7 principles. Since then I've been deep in CLI work, watching agents use them, and seeing them break in interesting ways.

Mid-April, Cloudflare published their work on rebuilding Wrangler around a TypeScript schema that generates the CLI, the SDKs, the Terraform provider, and the MCP server from one source. Their Code Mode MCP serves their entire ~3,000-operation API in under 1,000 tokens. They added `/cdn-cgi/explorer/api`, an OpenAPI-shaped runtime endpoint for agents. And they enforce naming rules across the entire CLI surface: always `get`, never `info`; always `--force`, never `--skip-confirmations`; always `--json`. Their framing: "manually enforcing consistency through reviews is Swiss cheese."

Shortly after, HeyGen launched a CLI built for agents. Generating videos through agents, polling jobs, routing artifacts to webhooks. The most agent-pleasant CLI I've used.

The original 7 principles were defensive: things a CLI has to get right, or agents pay for it on every call. Don't hang on a TTY check, return JSON, make errors actionable, bound the output. That layer is necessary but not enough.
...
The 10 principles below come from my own CLI work alongside what Cloudflare and HeyGen have published. Organized into 2 tiers. Five condense the original seven; five are new.

---

## Tier 1 — Don't break the agent

Agents are good at figuring things out, but when these aren't met the deck is stacked against them. Every gap costs more tokens, more retries, and more failure modes that don't surface until production.
...
- **Optimization target:** a comprehensive non-interactive mode the agent can rely on without per-command lookups.
...
- **Optimization target:** errors that include the valid set and a working invocation.
...
- **Optimization target:** bounded defaults that guide better queries plus an MCP surface where each tool's description fits in a tweet.
...
The principle I'm most certain about, and the one most under-stated in the original.
...
Cloudflare's schema-layer rules:
...
Their framing: "manually enforcing consistency through reviews is Swiss cheese." Vocabulary consistency has to be enforced mechanically, at the codegen or schema layer, because human review will always let edge cases slip.

The principle generalizes. Pick the convention the broader community already uses (Unix `--yes` for skip-prompt, `--limit` for pagination, the `get/list/create/update/delete` verb set), and don't deviate without strong reason. Where you do have to invent vocabulary because the concept is genuinely new, name it consistently across your own commands and document it once, prominently.
...
The original principle was "progressive help discovery": top-level `--help` lists commands, subcommand `--help` shows usage. That's still true, but it's now the bottom layer of a three-layer stack. Each layer answers a different question.
...
`--help` is necessary because some agents will hit it before anything else, and a human dropping into the terminal needs it. `agent-context` is what an introspecting agent should consume: versioned, machine-readable JSON describing the full shape. Cloudflare's `/cdn-cgi/explorer/api` is the runtime version; the CLI equivalent is a top-level subcommand. Ship `<cli> agent-context` with a `schema_version` field so the consuming agent can detect breaking shape changes.

The skill manifest is the third layer: long-form prose teaching the agent how to compose operations into useful workflows. HeyGen ships a directory of `SKILL.md` files alongside their CLI, and Cloudflare's MCP server is the equivalent.
...
- **Optimization target:** `--wait` on every async submission with a durable, recoverable ledger.
...
Precedence: explicit flag > environment variable > profile > default. Surfacing the available profile names in `agent-context` matters: it's how an introspecting agent discovers which identities exist without parsing a config file.
...
- **Optimization target:** named profiles with clean precedence, surfaced through `agent-context`.
...
`--deliver` routes the artifact directly: stdout, a file path, or a webhook URL. A video that lands as an MP4 at a known path, or POSTs to a webhook the agent already set up, is one fewer hop than "stdout to a temp file then move." File sinks write atomically; webhook sinks POST and surface HTTP status; unknown schemes return a structured refusal. HeyGen's framing for this: "fewer steps between agent output and a finished artifact."
...
- **Optimization target:** structured delivery and discoverable feedback, both versioned in introspection.

---

## Meta — enforce mechanically
...
The classic Unix CLIs treat a human at a terminal as the primary user, with agents as a tolerated secondary audience. That's no longer the right default. Cloudflare puts it directly: "Increasingly, agents are the primary customer of our APIs." Design for agents first, and humans benefit. Designing for humans first and bolting on agent support is what produces the inconsistent, prompt-prone, stdout-only CLIs the first five principles exist to correct.
